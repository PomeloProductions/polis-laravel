...
class TodoVacationPeriod extends BaseModelAbstract implements HasValidationRulesContract
{
     ...
     *
     * Compares CALENDAR DATES (Y-m-d strings), not instants: callers pass dates in the user's
     * timezone while start/end are stored as plain UTC dates, so instant comparison shifts the
     * boundary by the UTC offset (e.g. for UTC+2, midnight on the day AFTER end_date is still
     * 22:00 UTC on end_date — which wrongly extended vacations by a day and suppressed the first
     * post-vacation increment).
     */
